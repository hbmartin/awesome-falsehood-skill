...
- Missing decimal points caused 100x overcharges in postage/accounting anecdotes; the bug class is unit confusion, not just bad UI.
- A system can fail at unexpectedly large compensation, credit, coupon, or ledger values when upper bounds are assumed informally.
- UK company names with `<`, `>`, quotes, semicolons, ampersands, and SQL-looking strings demonstrate why output escaping cannot be skipped.
- CLDR currency validity ranges can overlap because political and monetary transitions are not clean database intervals.
